      ...
      for (const market of solanaMarkets) {
        ...
        if (commonTokens.includes(token)) {
          ...
          if (solBidNet > binanceAskNet) {
            ...
            const profitPer = (profit / binanceAskNet) * 100; // Calculate profit percentage
            opportunities.push({
              token,
              action: 'Buy on Binance, Sell on Solana',
              profit: profit.toFixed(2),
              profit_per: profitPer.toFixed(2), // Add profit percentage
              bestBid,  // debug info
              bestAsk,  // debug info
            });
          }
          ...
          if (binanceBidNet > solAskNet) {
            ...
            const profitPer = (profit / solAskNet) * 100; // Calculate profit percentage
            opportunities.push({
              token,
...
async function subscribeToBinanceWebSocketDynamic() {
    try {
      ...
      // Batch subscription into smaller groups if necessary
      const BATCH_SIZE = 5;
      for (let i = 0; i < streams.length; i += BATCH_SIZE) {
        ...
        ws.on('open', () => {
          ...
          console.log(`Subscribed to Binance WebSocket batch: ${batch}`);
        });
      }
      ...
      ws.on('message', (data) => {
        ...
        if (parsedData.s && parsedData.c) {
          ...
          console.log(`Updated Binance price for ${token}: ${price}`);
        }
      });
      ...
      ws.on('error', (err) => {
        ...
        if (err.message.includes('429')) {
          ...
          setTimeout(() => subscribeToBinanceWebSocketDynamic(), 5000);
        }
      });
      ...
      if (error.response && error.response.status === 429) {
        console.log('Server responded with 429 Too Many Requests. Retrying...');
        setTimeout(() => subscribeToBinanceWebSocketDynamic(), 5000);
      } else {
        console.log('Unhandled error. Retrying...');
        setTimeout(() => subscribeToBinanceWebSocketDynamic(), 10000);
      }
    }
  }
